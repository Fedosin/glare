    ...
    def create_database(self):
        ...
        if self.needs_database:
            ...
            if glare_db_env in os.environ:
                ...
                cmd = ('%s -m glare.cmd.db_manage upgrade' %
                       sys.executable)
...
class GlareServer(Server):
    ...
    def __init__(self, test_dir, port, policy_file, delayed_delete=False,
        ...
        self.server_module = 'glarce.cmd.%s' % self.server_name
...
pipeline = faultwrapper versionnegotiation unauthenticated-context rootapp

[pipeline:glare-api-fakeauth]
pipeline = faultwrapper versionnegotiation fakeauth context rootapp

[pipeline:glare-api-noauth]
pipeline = faultwrapper versionnegotiation context rootapp

[composite:rootapp]
paste.composite_factory = glare.api:root_app_factory
/: glarev1api
